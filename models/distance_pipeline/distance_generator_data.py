...
class DataGenerator(keras.utils.Sequence):
    ...
    def __init__(self, paths=None, max_prots=10, batch_size=8, crop_size=200, pad_size=200,
                 n_classes=5, class_cuts=[-0.5, 500, 1000, 1700], shuffle=True):
        'Initialization'
        # Get data
        self.names, self.seqs, self.dists, self.pssms = get_data(paths, max_prots)
    ...
    def get_data(self, paths, max_prots):
            ...
            for i,line in enumerate(lines):
                ...
                if line == "[ID]":
                    ...
                    if len(names)%100 == 0:
                        ...
                        try: logger.info("Currently @ ", len(names), " out of "+str(max_prots))
                        except:pass
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        inputs_pssm = np.array([self.wider_pssm(self.pssms[i], self.seqs[i])[r_i[k][0]:r_i[k][0]+self.crop_size, r_i[k][1]:r_i[k][1]+self.crop_size]
                                for k,i in enumerate(list_IDs_temp)])
        # print(self.inputs_aa.shape, self.inputs_pssm.shape)
